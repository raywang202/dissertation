# this solves the 'full' EM algorithm
...
DFDataSkilled['skilled_emax_mapping_abridged']=skilled_emax_mapping2(
...
# scales up values by subtracting out minimum value
        ...
        for dSTEM in range(2):
            for grade in LaborGradeRange:
                ...
                GenerateEmaxLaborFunctions(horizon,horizon_later,
                    ...
                    flows_skilled_by_type_penalized[endowment_type],wageshocks,
                    dSTEM,grade,choose,quality,np.array(switching_costs_skilled,
                        dtype=np.float64),wageshocks_later,zero_exp_penalty)
...
for endowment_type in range(num_endowment_types):
    ...
    plike[endowment_type,:]=np.sum(fs.LogLikeSkilled(DFDataSkilled,SATTuition,
...
# Look into the payouts for each individual, conditional on SAT_M, SAT_V, HS_GPA,
# and unobserved type
...
grade_params_by_quality = np.array([[xEd[0:7]+xEd[14:16],
    xEd[7:14]+xEd[16:18]],[xEd[0:3]+xEd[32:36]+xEd[18:20],xEd[7:10]+xEd[36:40]+
    xEd[20:22]]],
    dtype=np.float64)
...
for unobs_type in range(num_endowment_types):
    ...
    school_likes[unobs_type]=fs.LogLikeEducation(
        ...
        univ_type_shifters,grad_payoff,True)
...
# 3 types!
school_like_by_id = pd.DataFrame({'id':np.unique(DFDataEducation.index.values),
...
posteriors_total_arr=np.array([posteriors_total.type1,posteriors_total.type2,
    posteriors_total.type3])

posteriors_by_id=normalize_posterior(posteriors_total_arr,endowment_dist_old)
...
posteriors_skilled_subset = np.zeros([len(skilled_subset_by_id),num_endowment_types])
for i in range(num_endowment_types):
    posterior_frame=pd.DataFrame({'id':np.unique(DFData.index.values),
        'data':posteriors_by_id[i]})
    posterior_frame=posterior_frame.set_index('id')
    posteriors_skilled_subset[:,i]=np.array(skilled_subset_by_id.merge(posterior_frame,
        left_index=True,right_index=True).data)

# initial values, scaled
xLaborCopy=xLabor.copy()
for idx in [49,50,51,52,53,54,55,56,57,65,66,67,68,69,70,73,74,75]:
    xLaborCopy[idx]=xLaborCopy[idx]/10
for idx in [6,15,24]:
    xLaborCopy[idx]=xLaborCopy[idx]*10

x1Init = (xLaborCopy[0:30]+xLaborCopy[36:45]+xLaborCopy[49:58]+
    xLaborCopy[65:71]+xLaborCopy[73:76])
